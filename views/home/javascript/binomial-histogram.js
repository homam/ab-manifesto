...
(function(){
  var ref$, mean, sum, id, map, fold, filter, groupBy, objToPairs, head, tail, splitAt, zipAll, maximum, minimum, exports, drawExperimentNTries, drawHistogramAxes, drawHistogram, drawPathDiagram, drawDoubleHistogram;
  ref$ = require('prelude-ls'), mean = ref$.mean, sum = ref$.sum, id = ref$.id, map = ref$.map, fold = ref$.fold, filter = ref$.filter, groupBy = ref$.groupBy, objToPairs = ref$.objToPairs, head = ref$.head, tail = ref$.tail, splitAt = ref$.splitAt, zipAll = ref$.zipAll, maximum = ref$.maximum, minimum = ref$.minimum;
  ...
  drawHistogramAxes = function($svg, data, arg$){
    var format, ref$, xdomainf, ydomainf, duration, width, height, drawPercentageAxis, margin, x$, x, y, y$, $vp, z$, yAxis, z1$, $yAxis, bins, xAxis, z2$, $xAxis, xpAxis, z3$, $xpAxis;
    format = (ref$ = arg$.format) != null
      ...
      }, duration = (ref$ = arg$.duration) != null ? ref$ : 1000, width = (ref$ = arg$.width) != null ? ref$ : 600, height = (ref$ = arg$.height) != null ? ref$ : 260, drawPercentageAxis = (ref$ = arg$.drawPercentageAxis) != null ? ref$ : false;
    ...
    $svg.attr('width', width + margin.left + margin.right).attr('height', height + margin.bottom + margin.top);
    x$ = x = d3.scale.ordinal();
    x$.domain(xdomainf(data)).rangeBands([0, width], 0.1, 0);
    y = d3.scale.linear().domain(ydomainf(data)).range([height, 0]);
    y$ = $vp = $svg.selectAll('g.vp').data([data]);
    z$ = y$.enter().append('g').attr('class', 'vp');
    z$.append('g').attr('class', 'y axis');
    z$.append('g').attr('class', 'x axis');
    z$.append('g').attr('class', 'xp axis');
    ...
    z1$ = $yAxis = $vp.select('.y.axis');
    z1$.transition().duration(200).call(yAxis);
    bins = x.domain().length;
    xAxis = d3.svg.axis().scale(x).orient('bottom');
    z2$ = $xAxis = $svg.select('.x.axis').attr("transform", "translate(0," + height + ")");
    z2$.transition().duration(200).call(xAxis);
    z2$.selectAll('text').text(function(v, i){
      if (i % ceil(bins / 20) === 0) {
        return v;
      } else {
        return '';
    ...
    if (drawPercentageAxis) {
      ...
      z3$ = $xpAxis = $svg.select('.xp.axis').attr("transform", "translate(0," + (height + 25) + ")");
      z3$.transition().duration(200).call(xpAxis);
  ...
  drawHistogram = function($svg, data, arg$){
    var format, ref$, xdomainf, ydomainf, duration, width, height, drawPercentageAxis, $vp, x, y, x$, $block, y$, z$, z1$, expectedValue, n, ldata, xl, line, z2$;
    format = (ref$ = arg$.format) != null
      ? ref$
      : d3.format(","), xdomainf = (ref$ = arg$.xdomainf) != null
      ? ref$
