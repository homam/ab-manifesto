...
(function(){
  ...
  closestThen = curry$(function(o, a, ifa, b, ifb){
    if (abs(o - a) <= abs(o - b)) {
      return ifa;
    } else {
      return ifb;
  ...
  closestInList = curry$(function(o, xs){
    var find;
    if (empty(xs)) {
      return null;
    }
    ...
    find = function(result, arg$){
      var x, rest;
      x = arg$[0], rest = slice$.call(arg$, 1);
      if (empty(rest)) {
        return closest(o, result, x);
      }
      return find(closest(o, result, x), rest);
    };
    return find(xs[0], tail(xs));
  });
  closestInListWithTransform = function(o, cSelector, vSelector, xs){
    var find;
    if (empty(xs)) {
      return null;
    }
    if (xs.length === 1) {
      return [cSelector(xs[0]), vSelector(xs[0])];
    }
    find = function(arg$, arg1$){
      var c, v, x, rest;
      c = arg$[0], v = arg$[1];
      x = arg1$[0], rest = slice$.call(arg1$, 1);
      if (empty(rest)) {
        return closestThen(o, c, [c, v], cSelector(x), [cSelector(x), vSelector(x)]);
      }
      return find(closestThen(o, c, [c, v], cSelector(x), [cSelector(x), vSelector(x)]), rest);
    };
    return find([cSelector(xs[0]), vSelector(xs[0])], tail(xs));
  };
  toss = function(){
    return round(random());
  };
  ...
  exports.closestThen = closestThen;
  exports.closestInList = closestInList;
  exports.closestInListWithTransform = closestInListWithTransform;
  exports.toss = toss;
  exports.trial = trial;
